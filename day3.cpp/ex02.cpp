...
int steps(vector<Point> &vec, Point pt) {
    for(int i=0; i<vec.size(); i++) {
        if(vec[i] == pt) return i;
    }
}
...
int main()
{
    ...
    vector<Point> unordered1(pointVec1);
    vector<Point> unordered2(pointVec2);
    ...
    for(int i=0; i<collisions.size(); i++) {
        Point inters = collisions[i];
        int steps1 = steps(unordered1, inters);
        int steps2 = steps(unordered2, inters);
        int stepSum = steps1 + steps2;
        if(!stepSum) continue;
        if(stepSum < min_step) min_step= stepSum; 
    }
    cout<<"Num min di step: "<<min_step;
